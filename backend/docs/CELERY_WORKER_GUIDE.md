...
| `POST /media-assets` (이미지 업로드 시 자동) | `analyze_media_asset` | Document Parse 동기 API 호출 + Solar 타당성 검증(Phase 1 듀얼 트랙 분석). 별도 엔드포인트가 아니라 업로드 응답을 반환한 직후 자동으로 큐잉된다 — `app/services/media_service.py` 상단 docstring 참조 |
