...
- 정상 기동 시 콘솔에 5개 태스크가 등록된 것이 보여야 한다:
  `process_session_completion`, `consolidate_autobiography`, `write_chapter`,
  `finalize_manuscript`, `generate_manuscript_pdf`.
- `generate_manuscript_pdf`가 등록조차 안 되고 워커 시작 자체가 죽는다면 GTK3 런타임이
  ...
  프로세스 전체가 못 뜬다(PDF 기능만 못 쓰는 게 아니라 5개 태스크 전부 등록 실패).
...
`registered`에 위 5개 태스크가 보이고 `ping`이 `pong`을 반환하면 준비 완료다. 이제 백엔드
