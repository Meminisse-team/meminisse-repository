...
로직(`prompts.contains_crisis_keyword`)을 쓴다.

2026-07-12부터 1층도 실제 서비스에 연결됐다(`interview_service._detect_strong_negative_
emotion` — 위기 키워드는 아니지만 심화 질문을 피해야 할 만큼 강한 부정적 감정을 저비용
LLM 분류로 판별). 다만 이 샌드박스 엔드포인트는 여전히 그 판별 없이 **위기 키워드가
없으면 무조건** 1층 완충 프롬프트를 호출한다 — 프롬프트 자체를 빠르게 확인하기 위한
용도이지, 실제 트리거 조건까지 재현하지는 않는다는 점에 유의.
...
### 20. `POST /sandbox/sample-preview` — 자서전 커스터마이징 샘플 미리보기

**목적**: 자서전 커스터마이징 기능에 사용되는 8개 맛보기 텍스트 생성 프롬프트(`SAMPLE_PREVIEW_SYSTEM_PROMPT`)를 검증한다. 실제 서비스에서는 사용자가 선택한 1~2개의 말투/구성/컨셉 조합을 순회하며 비동기 큐에서 최대 8번 호출하지만, 이 샌드박스에서는 조합 1개씩 자유롭게 호출하여 스타일 반영 결과를 비교해볼 수 있다.

**입력 (`SamplePreviewSandboxRequest`)**

| 필드 | 타입 | 필수 | 설명 |
| --- | --- | --- | --- |
| `tone_key` | `string` | ✅ | 말투 키 (예: `"plain"`) |
| `structure_key` | `string` | ✅ | 구성 키 (예: `"chronological"`) |
| `concept_key` | `string` | ✅ | 컨셉 키 (예: `"complete_memoir"`) |
| `style_bible` | `string` | ✅ | 스타일 바이블 텍스트 |
| `event_summaries` | `string` | ✅ | 샘플 생성에 쓰일 10개 내외의 사건 요약 문자열 |
| `system_prompt_override` | `string \| null` | ❌ | — |
| `generation` | `GenerationOverrides \| null` | ❌ | `reasoning_effort` 기본값: `"medium"`. |

**출력 (`SamplePreviewSandboxResponse`)**: `messages_sent`, `preview_text`(생성된 맛보기 텍스트), `tone_name`, `structure_name`, `concept_name`

---

### 21. `POST /sandbox/customized-chapter-writing` — 커스터마이징된 챕터 집필

**목적**: 말투(tone)와 컨셉(concept) 지시문이 주입된 `CHAPTER_WRITING_SYSTEM_PROMPT`를 검증한다. 기존 14번 챕터 집필 샌드박스와 동일한 입력 구조에 `tone_key`, `concept_key`가 추가되었다.

**입력 (`CustomizedChapterWritingRequest`)**
...
| `retrieved_event_paragraphs` | `string[]` | ✅ | — |
| `system_prompt_override` | `string \| null` | ❌ | — |
| `generation` | `GenerationOverrides \| null` | ❌ | `reasoning_effort` 기본값: `"high"`. |

**출력 (`CustomizedChapterWritingResponse`)**: `messages_sent`, `chapter_content`(생성된 챕터 본문), `tone_name`, `concept_name`
