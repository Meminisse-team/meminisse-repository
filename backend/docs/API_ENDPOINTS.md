...
- **Upstage API 오류 처리**: Solar/Embeddings 호출(OpenAI SDK 호환, `openai.APIStatusError`)이
  실패하면(잘못된 키, 요금 초과, 모델 비호환 등) `app/main.py`에 등록된 전역 핸들러가 원본
  트레이스백 대신 `{"detail": "Upstage API 오류: ..."}` 형태의 JSON을 Upstage가 반환한 HTTP
  상태 코드 그대로 돌려준다. 이 오류는 아래 각 엔드포인트 설명에서 "Upstage 호출 실패 시"
  공통으로 적용되므로 항목마다 반복하지 않는다. 순수 httpx로 별도 처리하는 외부 API
  (Azure Vision, Supabase Auth 등)는 이 핸들러 대상이 아니다 — `openai.APIStatusError`만
  잡는다.
...
확인했다 — 즉 이메일 인증·비밀번호 재설정까지 지원하는 완전한 인증
...
계정 생성 시점에 받지 못한 프로필 필드를 나중에 채우거나, 일반 프로필 수정에
그대로 쓸 수 있게 범용으로 설계했다.
...
### `POST /api/v1/interview-sessions/skip-next` — 다음 질문 건너뛰기 (인증 필요, 2026-07-16 추가)

미리보기(`GET /next-preview`)로 보여준 다음 질문/사진을 사용자가 거부('이 질문
넘어가기')한 경우 — 아직 세션이 없는 상태 전용. 서버가 같은 멱등 계산으로 그 항목을
다시 구해 `status="skipped"` 세션으로 배정 처리하므로, 그 질문(또는 사진)은 다시
후보에 오르지 않는다. 동적 질문 필터링의 자동 스킵과 같은 메커니즘을 사용자 트리거로
노출한 것이다.

**요청 바디**: 없음.

**응답 `200 OK` (`NextItemPreviewRead`)**: 건너뛴 **다음** 항목의 새 미리보기 —
프론트가 별도 왕복 없이 다음 질문을 바로 이어 보여줄 수 있다. 더 건너뛸 항목이
없으면 `409 Conflict`.

---

### `POST /api/v1/interview-sessions/{session_id}/skip` — 열린 세션 건너뛰기 (인증 필요, 본인 세션만, 2026-07-16 추가)

이미 만들어진(열린) 세션의 질문을 사용자가 거부한 경우. `complete`와 달리 **Phase 2
후처리(산문 재조립·이벤트 추출)를 큐잉하지 않고** `status="skipped"`로만 전이한다 —
거부한 질문의 대화는 자서전 재료가 아니며, '나의 이야기' 목록에서도 제외된다.

**응답 `200 OK` (`SessionRead`)**: `status="skipped"` 반영. 이미 종료된 세션이면
`409 Conflict`.
...
안에서 외부 동기 API 호출까지 전부 끝내고서야 응답했는데, 사진 한 장 올리는 데
오래 걸리거나 외부 API 쪽 일시적 오류로 업로드 자체가 500으로 실패하는 문제가
있었다 — "사진을 올려도 처리가 끝나지 않는" 것처럼 보였다).

**2026-07-15**: 분석 엔진을 Upstage Document Parse(텍스트만 읽는 OCR)에서 Azure
AI Vision Image Analysis로 교체했다 — 글자가 없는 순수 추억 사진에서는 Document
Parse가 아무 단서도 못 얻었는데, Azure Vision은 `features=caption,read` 한 번의
호출로 캡션(사진의 시각적 내용을 설명하는 문장)과 사진 속 텍스트를 함께 지원해서
텍스트 유무와 무관하게 항상 쓸만한 단서를 얻는다(`app/clients/azure_vision.py`).
...
2. (요청 밖, Celery `analyze_media_asset` 태스크) S3에서 방금 올린 원본을 다시 내려받아
   `azure_vision.analyze_image(...)`로 캡션 + 사진 속 텍스트를 한 번의 호출로 추출한다.
   `AZURE_CV_ENDPOINT`/`AZURE_CV_API_KEY`가 아직 `.env`에 설정되지 않았으면
   (`AzureVisionNotConfiguredError`) 분석 자체를 건너뛴다 — 업로드는 이미 성공했으므로
   앱이 죽지 않고, `analysis_track`/`image_caption`/`image_ocr_text` 모두 `null`로 남는다.
3. 캡션은 텍스트 유무와 무관하게 `image_caption`에 항상 저장된다. 검출된 텍스트가
   **20자 미만**이면 `analysis_track="pure_memory"`로 확정하고 종료 — 이벤트는
   생성되지 않는다(캡션 + 사용자 코멘트가 유일한 문맥 자료가 된다).
4. 20자 이상이면 `analysis_track="text_document"`로 확정, `image_ocr_text`에 저장하고
   Azure Vision 원시 응답을 `pre_extracted_labels`에 캐시 저장. 이어서 Solar Structured
   Outputs로 1차 타당성 검증(오인식/깨진 텍스트 의심 여부)을 수행한다.
5. 검증 결과에 따라 `Event(source_type="document")`를 생성한다: 의심 없음(`suspicious=false`)이면
   ...
   임베딩은 생성하지 않는다(RAG에서 완전히 제외됨 — Layer 1 검증 게이트). 이 사건은 더
   이상 "대기 중 확인" 상태로 특별 취급되지 않는다 — 나중에 그 사진에 대해 실제 대화가
   오가 비슷한 사실이 다시 추출되면 Phase 3 중복 병합(`_merge_duplicate_events`)이
   임베딩 유사도 + LLM 판정으로 자연스럽게 흡수한다.
...
image_caption, image_ocr_text, user_comment, created_at` — 이 시점에는 `analysis_track`이
아직 `null`일 수 있다(분석이 비동기로 진행 중이라는 뜻). 목록(`GET /api/v1/media-assets`)을
다시 조회하면 채워진 값을 볼 수 있다.
...
**주의할 점**:
- 오인식/깨진 텍스트로 의심되어 격리된(`verified=false`) 사건은 이 목록에 나타나지
  않는다 — 검증되지 않은 내용을 "내 이야기"로 보여주지 않기 위한 의도적 설계다.
...
toc_data, style_bible, book_synopsis, final_content, pdf_url, photo_placements, created_at, updated_at`
...
### `PUT /api/v1/autobiographies/{autobiography_id}/photo-placements` — 수록 사진 배치 지정 (2026-07-16 추가)

PDF 조판 직전, 자서전에 실을 사진과 위치를 사용자가 직접 고른 결과를 저장한다.
기획안 5절(고정 슬롯 템플릿) 원칙에 따라 위치는 자유 좌표가 아니라 **챕터 + 슬롯**
조합으로만 표현한다.

**요청 바디 (`PhotoPlacementsUpdate`)**: `placements` 배열 — 각 항목은
`media_asset_id`(본인 소유 이미지여야 함), `chapter_index`(실존 챕터여야 함),
`slot`(`"chapter_top"` 챕터 첫머리 | `"full_page_before"` 챕터 앞 전면 사진 페이지),
`caption`(선택). PUT 시맨틱 — 보낸 배열이 전체 상태다. **빈 배열은 "수록 사진
없음"으로 확정**하는 것. 조판은 여기 지정된 사진만 수록하며, 저장한 적이 없어도
(null) 사진 없이 조판된다 — 자동 선택은 없다(2026-07-17 폴백 제거).

**응답 `200 OK` (`AutobiographyRead`)**: `photo_placements` 반영. 유효하지 않은
사진/챕터를 가리키면 `400`.
...
1. Jinja2로 표지(제목·전체 시놉시스)·목차·챕터별 본문을 고정 슬롯 템플릿
   (`app/templates/manuscript.html.jinja`)에 주입한다. 사진은 사용자가
   `photo-placements`로 지정한 것(챕터 첫머리/전면 페이지 슬롯)만 수록한다 —
   미지정(null)이든 빈 배열이든 사진 없이 조판되며, 챕터 소환 이벤트 사진을
   자동 배치하던 동작은 제거됐다(2026-07-17). **주의**: 챕터 본문은 `final_content`(윤문된 단일
...
**전제 조건**: `final_content`가 비어 있으면(=`finalize`를 아직 안 했으면) 큐잉하지 않고
즉시 `409 Conflict`를 반환한다(2026-07-16 — 이전엔 태스크 내부 `ValueError`로만 실패해
HTTP로 전달되지 않았다). `write`/`finalize`도 같은 방식의 선행 조건 409를 반환한다.
...
| 한계 | 위치 | 영향 |
| --- | --- | --- |
| 왜곡 판정 세션의 후속 처리 미구현 | `event_extraction_service.process_completed_session` | NLI 왜곡 탐지 자체는 동작하지만, 임계값 초과 시 재조립 재시도나 "검증 보류" 플래그 노출 없이 이벤트 추출만 조용히 보류된다(코드 내 TODO) |
| 팩트체크의 인명 별칭 매핑 부재 | `autobiography_service._run_factcheck` | 지명 정규화·나이→절대연도 환산은 구현됐으나, 인명은 대소문자 무시 부분 문자열 매칭에 머문다 — 가족 호칭·애칭("우리 큰애"="영수")을 같은 인물로 잇는 본격적 개체 연결(entity linking)이 필요해 범위 밖으로 남겼다. `quantity` 타입 팩트도 대조 불가 |
...
2026-07-16 점검에서 표가 코드보다 뒤처져 있던 3건을 정리하고(왜곡 탐지·근거검증은
자리표시자가 아니라 실제 NLI 로컬 추론으로 이미 교체돼 있었고, 팩트체크도 재추출-
정규화-대조로 구현돼 있었다 — 남은 잔여 한계만 표에 축소 반영), 2건을 실제로 해소했다:
- **위기 대응 시 이벤트 추출 미큐잉** → `add_user_turn` 위기 분기가 커밋 직후
  `process_session_completion`을 직접 큐잉한다. 이전엔 상태만 completed로 바뀌어
  그 세션의 이야기가 '나의 이야기'에 영영 "생성 중" placeholder로 남았다.
- **Phase 4/5 202 엔드포인트의 선행 조건 실패가 HTTP로 전달 안 됨** →
  `chapters/{id}/write`(목차 미선택), `finalize`(챕터 미집필), `pdf/generate`
  (윤문 미완료)가 각각 사전 조회로 409를 즉시 반환한다. 워커 쪽 검증은 큐잉 이후
  상태 변화에 대비한 이중 방어로 유지.
...
2026-07-15에 사진 분석 엔진을 Upstage Document Parse(텍스트만 읽는 OCR)에서
**Azure AI Vision Image Analysis로 교체**했다 — 글자가 없는 순수 추억 사진에서는
Document Parse가 아무 단서도 못 얻어 일반적인 오프닝 질문만 던질 수 있었는데,
Azure Vision은 `features=caption,read` 한 번의 호출로 캡션(사진의 시각적 내용을
설명하는 문장)과 사진 속 텍스트를 함께 지원해서 텍스트 유무와 무관하게 항상
구체적인 단서를 얻는다(`app/clients/azure_vision.py`, `MediaAsset.image_caption`/
`image_ocr_text` 신규 컬럼, 마이그레이션 010). `document_parse.py` 클라이언트는
다른 호출부가 없어 완전히 제거했다. 이 교체와 함께 "OCR 확인 대기" 이벤트를 별도
상태로 추적하던 `EventGateway.get_pending_document_confirmation`/`delete`도
제거했다 — 캡션/텍스트가 이제 `MediaAsset`에 직접 저장되어 더 이상 Event를 거쳐
조회할 필요가 없고, 사진 속 텍스트로 스테이징한 사건은 PHOTO 세션 완료 시 더 이상
특별히 삭제하지 않고(일반 이벤트와 동일하게 취급) Phase 3 중복 병합이 필요하면
자연스럽게 흡수하도록 단순화했다. 상세는 `docs/QUESTION_BANK_GUIDE.md` 5절
"사진 캡션 + 텍스트 인식 — Azure Vision으로 교체" 참조.
