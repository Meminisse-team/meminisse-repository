...
# 지터 폭(초) — 동시 처리(2026-07-19 도입) 중 여러 세션이 같은 순간 429를 맞으면
# 지터 없는 고정 백오프는 재시도도 똑같은 타이밍에 몰려 두 번째 429 무더기를
# 만든다("thundering herd"). 다만 이건 재시도가 서로 겹치는 것만 완화할 뿐,
# 동시 요청 자체가 Solar의 실제 처리량 한도를 넘어서는 근본 문제는 해결하지
# 않는다 — 동시성 값(DEFAULT_CONCURRENCY)을 보수적으로 잡은 이유이기도 하다.
_RETRY_JITTER_SECONDS = 3.0
# 순차 처리(세션 100개 완전 직렬 실행) 시 세션당 실측 처리 시간(30~70초, 워커
# 로그 실측)을 그대로 곱하면 100세션에 30~80분이 걸린다(2026-07-19 실사용 중
# 확인) — 세션끼리 서로 독립적이라 순서를 지킬 이유가 없으므로 동시 처리로
# 바꾼다. evals/followup_trigger_audit.py가 동시성 5에서 32/100 429를 겪은
# 전례가 있어, 그보다 보수적인 값을 기본으로 삼는다.
...
async def _process_one_session(session_id: uuid.UUID) -> int:
    ...
    for attempt in range(_RETRYABLE_MAX_ATTEMPTS):
        try:
            ...
            print(f"    [429 재시도 {attempt + 1}/{_RETRYABLE_MAX_ATTEMPTS}] {delay:.1f}초 대기 (session={session_id})...", file=sys.stderr)
...
async def _process_one_session_logged(
    *, index: int, total: int, session_id: uuid.UUID
) -> tuple[uuid.UUID, int | None, Exception | None]:
    """`_process_one_session`을 감싸 진행 로그를 남기고 결과를 튜플로 반환한다.
    동시 처리 중에는 완료 순서가 뒤섞여 로그가 인터리브되므로(2026-07-19),
    매 줄에 session_id를 반드시 남겨 어느 세션의 로그인지 항상 구분 가능하게
    한다. processed/failed 집계는 여기서 직접 하지 않고 반환값을 호출부가
    asyncio.gather 이후 한 번에 세도록 한다 — 동시 실행 중인 코루틴들이 각자
    공유 카운터를 직접 증가시키는 구조를 피해 집계 로직을 항상 단순하고
    명확하게 유지하기 위함이다."""
    print(f"  [{index}/{total}] session={session_id} 처리 시작...", file=sys.stderr)
    try:
        count = await _process_one_session(session_id)
        print(f"  [{index}/{total}] session={session_id} → 이벤트 {count}건 추출 완료", file=sys.stderr)
        return session_id, count, None
    except Exception as exc:  # noqa: BLE001 — 세션 하나 실패해도 나머지는 계속(자기 트랜잭션만 롤백됨)
        print(f"  [{index}/{total}] session={session_id} ❌ 실패: {exc!r}", file=sys.stderr)
        return session_id, None, exc


async def _process_all_sessions(pending_ids: list[uuid.UUID], *, concurrency: int) -> tuple[int, int]:
    """세션들을 동시성 상한(세마포어)을 두고 병렬 처리한다 — 세션끼리 서로
    독립적이라(진행 순서를 지킬 이유가 없음) 완전 순차 처리는 불필요한 지연일
    뿐이었다(2026-07-19, 세션 100개 순차 처리가 30~80분 걸리던 문제 해소).
    동시성 값은 evals/followup_trigger_audit.py가 5에서 32/100 429를 겪은
    전례보다 보수적으로 기본값을 잡았다(_DEFAULT_CONCURRENCY 참조)."""
    semaphore = asyncio.Semaphore(concurrency)

    async def _bounded(index: int, session_id: uuid.UUID):
        async with semaphore:
            return await _process_one_session_logged(
                index=index, total=len(pending_ids), session_id=session_id
            )

    results = await asyncio.gather(
        *(_bounded(i, session_id) for i, session_id in enumerate(pending_ids, start=1))
    )

    processed = sum(1 for _, count, exc in results if exc is None)
    failed = sum(1 for _, count, exc in results if exc is not None)
    return processed, failed


async def main(email: str, *, repair: bool, concurrency: int = _DEFAULT_CONCURRENCY) -> None:
    async with gateways_context() as gateways:
    ...
    print(
        f"[대상] {user_name} ({email}) — 세션 {len(sessions)}개 중 미처리 "
        f"{len(pending_ids)}개 (동시성 {concurrency})"
    )

    processed, failed = await _process_all_sessions(pending_ids, concurrency=concurrency)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help=(
            f"동시에 처리할 세션 수(기본값 {_DEFAULT_CONCURRENCY}). 429가 잦으면 낮추고, "
            "여유가 있으면 올려서 실험해볼 것 — evals/followup_trigger_audit.py는 5에서 "
            "32/100 429를 겪었다."
        ),
    )
    args = parser.parse_args()
    asyncio.run(main(args.email, repair=args.repair, concurrency=args.concurrency))
