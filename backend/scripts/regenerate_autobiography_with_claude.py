"""
ordinary@dummy.com 등 특정 계정의 진행 중(미완성) 자서전을 Phase 3(스타일 바이블·목차)
부터 전부 재생성하되, 집필 파이프라인(app/services/autobiography_service.py,
app/services/character_service.py)의 LLM 호출만 Solar 대신 Claude로 라우팅해 비교해보기
위한 일회성 스크립트다(app/clients/llm_router.py 참조, scripts/regenerate_autobiography_
with_gemini.py의 Claude 버전).

.env의 AUTOBIOGRAPHY_LLM_PROVIDER 기본값("solar")은 건드리지 않는다 — 이 스크립트
프로세스 안에서만 settings.AUTOBIOGRAPHY_LLM_PROVIDER를 "claude"로 잠깐 덮어쓴다(런타임
값 변경이라 다른 프로세스·다음 실행에는 영향이 없다). .env에 ANTHROPIC_API_KEY가 없으면
첫 LLM 호출에서 바로 실패한다.
...
실행 방법 (backend/ 디렉토리에서):
    ..\\venv\\Scripts\\python -m scripts.regenerate_autobiography_with_claude --email "ordinary@dummy.com"
"""
...
settings.AUTOBIOGRAPHY_LLM_PROVIDER = "claude"  # 이 프로세스 안에서만 유효 — .env는 그대로 "solar".
...
_STAGE_TIMEOUT_SECONDS = 900  # Claude 응답 지연(adaptive thinking 포함) 대비 넉넉한 여유.
...
async def main(email: str, *, candidate_index: int, concurrency: int) -> None:
    ...
    print("[2/5] Phase 3(스타일 바이블·이벤트 병합·중요도) 재생성 중 (Claude)...", file=sys.stderr)
    async with gateways_context() as gateways:
        await asyncio.wait_for(
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="지정 계정의 진행 중 자서전을 Phase 3부터 Claude로 전부 재생성합니다."
    )
