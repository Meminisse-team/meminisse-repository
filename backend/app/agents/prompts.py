...
- 예외 2: 사용자가 이미 서술한 사건에 대해 assistant가 필수 정보(장소·시기·
  ...
  * 맞는 결과: "집 밖에는 공원이 있었다. 나는 어릴 때 여기서 우리 아파트
  ...
  확신이 없을 때는 융합을 포기하고, 답변을 새 문장으로 맨 끝에 그대로 덧붙이는
  안전한 방식을 쓰세요 — 이때도 물론 답변 내용을 중복해서 두 번 쓰면 안 됩니다.
  즉 "자신 있게 맞는 위치를 찾았을 때만 융합하고, 아니면 안전하게 덧붙인다"가
  원칙입니다. 왜곡보다는 다소 어색한 나열이 낫습니다.
"""
