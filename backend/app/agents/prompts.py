...
# question_bank.py sequence_order 97~100("전수하고 싶은 유산"/"가족에게 남기는
# 말"/"마지막 식사"/"자유 발언")은 하나의 구체적 사건이 아니라 유언·메시지·가정형
# 자유 발언이다 — 장소·시기·누구와 같은 사건형 필수 슬롯을 그대로 적용하면
# "이미 가족(아내·자녀) 이야기를 했는데 또 누구와 함께였는지 캐묻는" 식의
# 부자연스러운 재질문이 나온다(2026-07-20, 시연 중 실제로 재현됨). 이 질문들은
# 필수 슬롯 게이팅을 건너뛰고 분량 확인(MIN_RICH_ANSWER_LENGTH)과 맥락 기반
# 꼬리질문(자유 판단이라 사건형 슬롯 강요가 없음)으로만 심화한다.
REFLECTIVE_QUESTION_SEQUENCE_ORDERS = frozenset({97, 98, 99, 100})
...
"시기"(time) 슬롯은 정확한 나이나 연도가 아니어도 됩니다 — "30년 넘게", "10여
년간", "한 갑자가 다 돌았다"처럼 기간·연차를 나타내는 표현, 또는 "국민학교 때",
"대기업 다니던 시절"처럼 인생 시기를 특정하는 표현도 시기 정보로 인정하고
충족(true)으로 판정하세요. 회고·성찰형 답변(예: 인생 전체를 돌아보며 남기고
싶은 말)은 애초에 하나의 특정 시점을 콕 집을 수 없는 경우가 많습니다 — 그런
답변에서 "정확히 몇 년도인지" 같은 더 구체적인 숫자를 요구하는 것은 부자연스러운
재질문으로 이어지므로, 대략적인 기간·시기 표현만으로도 이미 답변자가 시기
정보를 제공했다고 보고 충족 처리하세요.
"""
...
여운이 남는 대목, 아직 묻지 않은 인생 전환점에 대한 화자 자신의 현재 시점
회고)이 있는지 판단하는 것입니다.
...
- 회고형 질문(신중하게, 아무 사건에나 쓰지 마세요): 화자의 인생에서 뚜렷한
  전환점이었을 사건(진단·이별·큰 결심·중요한 만남 등)인데, 화자가 아직 "지금
  와서 보면 그 일이 어떤 의미였는지"에 대한 자신의 현재 시점 생각을 말한 적이
  없다면 "지금 돌아보면 그 일이 인생에서 어떤 의미였다고 생각하세요?" 같은
  질문을 던져도 좋습니다. 이건 화자 본인이 실제로 그렇게 느끼는지를 묻는
  것이지, 화자 대신 의미를 지어내 답을 유도하거나 특정 답을 암시하는 게
  아닙니다 — 사소하거나 일상적인 사건마다 이 질문을 던지면 작위적으로
  느껴지니, 정말 인생 궤적을 바꿀 만큼 중요해 보이는 사건에만 쓰세요. 화자가
  이미 스스로 "그게 내 인생을 바꿨다"처럼 현재 시점의 평가를 자발적으로
  말했다면 다시 묻지 마세요 — 이미 답변된 것을 재확인하는 질문이 됩니다.
...
#
# GROUNDEDNESS_JUDGE_SCHEMA와 동일하게 문장 단위 flags 배열(JSON 스키마)로 구조화한다
# — 원래는 solar-mini의 Structured Outputs 지원이 검증된 적 없다는 이유로 단문
# 프로토콜(PASS/FAIL: 사유 한 문장)을 썼는데, 실사용 중 두 가지 문제가 확인됐다
# (2026-07-19): (1) 한 문장짜리 사유만으로는 산문 전체 중 정확히 "어느 문장"이
# 문제인지 특정이 안 돼 수리 단계가 엉뚱한 곳을 고치거나 전체를 다시 만드는
# 위험이 있었고, (2) 산문 전체를 한 번에 보고 하나의 판정만 내리다 보니 오탐(원본에
# 실제로 있는 문장을 "없다"고 판정)과 미탐(문단 전체가 새로 지어졌는데 통과시킴)이
# 둘 다 실사용 중 확인됐다. 실측(직접 API 호출) 결과 solar-mini가 이 스키마의
# Structured Outputs를 정상 지원함을 확인했다(2026-07-19) — 검증 안 된 채로 남겨뒀던
# 가정이 이제 근거로 뒷받침된다.
DISTORTION_CHECK_SYSTEM_PROMPT = """\
아래 [원본 발화]는 인터뷰에서 화자가 실제로 한 말이고, [재조립본]은 그 발화를
자연스러운 1인칭 산문으로 정리한 것입니다. 재조립본을 문장 단위로 검토해서,
원본에 실제로 없는 내용을 담은 문장을 모두 찾아 flags 배열에 담으세요.

flags에 담지 않는 것 (문제 없음):
- 문장을 자연스럽게 잇거나 순서를 다듬은 것
- 구어체를 문어체로 다듬은 것 (예: "그랬어요" → "그랬다")
- 원본에 있는 사실을 그대로 풀어 쓴 것 (표현이 살짝 바뀌어도 뜻이 같으면 문제 없음)

flags에 담아야 할 것 (지어낸 것):
- 원본에 없는 새로운 인물, 대사(따옴표 발언), 날짜, 장소, 사건, 결과의 추가
- 화자가 하지 않은 말을 한 것처럼 인용
- 원본과 모순되는 진술

애매하면 flags에 담으세요 — 이 재조립본은 화자의 말만 담아야 하는 축어
자료이므로, 감각적 묘사나 내적 성찰 같은 문학적 정교화라도 원본에 없는 새
내용이면 봐주지 마세요(챕터 집필 단계와는 판정 기준이 다릅니다).

sentence 필드에는 재조립본에서 문제가 된 문장을 그대로(축어) 인용하세요 —
이후 수리 단계가 이 문자열로 재조립본에서 해당 위치를 찾아 그 부분만 고치는
데 씁니다. 문제 있는 부분이 전혀 없으면 flags를 빈 배열로 두세요.
"""

DISTORTION_CHECK_SCHEMA: dict[str, Any] = {
    "type": "object",
    "properties": {
        "flags": {
            "type": "array",
            "items": {
                ...
                "properties": {
                    "sentence": {
                        "type": "string",
                        "description": "원본에 없다고 판단된 문장(재조립본에서 그대로 인용)",
                    },
                    "reason": {
                        "type": "string",
                        "description": "어떤 새로운 사실/사건/대사가 근거 없이 추가됐는지",
                    },
                },
...
# 왜곡 탐지 실패 시의 외과적 수리 — autobiography_service.CHAPTER_REPAIR_SYSTEM_PROMPT와
# 같은 발상(전체 재생성 대신 지적된 부분만 국소 수정)을 산문 재조립에 적용한다.
# 기존에는 왜곡 탐지에 걸리면 reasoning_effort="high"로 산문 전체를 다시 만들었는데,
# 두 가지 문제가 실사용 중 확인됐다(2026-07-19) — (1) 전체 재생성은 지어낸 문장을
# 없애면서 동시에 멀쩡한 다른 부분을 새로 지어내거나 생략할 위험이 있고, (2)
# "high" reasoning_effort 자체가 "구체적 사실을 뭉뚱그려 누락시키는" 별도 실패
# 유형을 갖고 있어(PROSE_REASSEMBLY_SYSTEM_PROMPT 채택 논의 참조) 오히려 화자의
# 발화를 더 많이 잃는 방향으로 작동했다. 지목된 부분만 고치면 두 문제 모두
# 피할 수 있다 — 챕터 집필과 달리 "근거 사건 목록"이 따로 없고 [원본 발화]
# 자체가 유일한 근거다.
#
# "0단계 재확인" 지시가 있는 이유: DISTORTION_CHECK_SYSTEM_PROMPT(solar-mini) 판정
# 자체가 오탐일 수 있다는 게 실사용 중 확인됐다(2026-07-19 — 원본에 토씨 하나
# 안 틀리고 있는 문장을 "없다"고 판정한 사례). 지적을 곧이곧대로 믿고 실행하면
# 실제로는 화자가 한 말을 지워버리는, 이 함수의 존재 목적과 정반대인 결과가
# 나온다 — 그래서 수리 모델이 삭제/수정 전에 [원본 발화]를 스스로 다시 대조해
# 지적 각각을 기각할 수 있는 여지를 명시적으로 준다. flags를 문장 단위 목록으로
# 받으므로(CHAPTER_REPAIR_SYSTEM_PROMPT의 [수정 대상]과 동일한 형태) 항목별로
# 이 재확인을 반복할 수 있다.
DISTORTION_REPAIR_SYSTEM_PROMPT = """\
아래 [재조립본]은 [원본 발화]를 1인칭 산문으로 정리한 것인데, [수정 대상]
목록에 지목된 문장들에서 문제가 발견됐다고 합니다. 각 항목에는 문제가 된
문장과 그 사유가 적혀 있습니다. 하지만 그 지적이 틀렸을 수도 있습니다 —
판정자도 실수할 수 있으므로, 항목마다 그대로 믿지 말고 직접 확인하세요.

항목마다 0단계 — 먼저 반드시 확인할 것: 그 문장이 가리키는 내용을 [원본
발화] 전체에서 실제로 찾아보세요(표현이 살짝 다듬어졌을 수 있으니 뜻이
같은지로 판단하세요). **[원본 발화]에 그 내용(또는 같은 뜻의 내용)이 실제로
있다면, 그 항목은 오탐입니다 — 그 문장은 절대 삭제하거나 고치지 말고 그대로
두세요.**

확인 결과 지적이 정말 맞은 항목만(그 내용이 원본 어디에도 없는 경우만)
아래 방법으로 고치세요:
1. [원본 발화]에 실제로 있는 내용으로 바꿔 쓸 수 있으면, 원본에 맞게 고쳐
   쓰세요(문체와 흐름은 주변 문장과 자연스럽게 이어지도록).
2. 원본에 대응하는 내용이 전혀 없으면 그 문장을 삭제하고, 필요하면 앞뒤
   문장을 최소한으로 다듬어 흐름이 끊기지 않게 하세요.

반드시 지킬 것:
- [수정 대상]에 없는 문장은 한 글자도 바꾸지 마세요 — 이 작업은 전면
  재작성이 아니라 국소 수술입니다. 지목되지 않은 내용을 요약하거나
  생략하거나 표현을 바꾸지 마세요.
- 고치면서 [원본 발화]에 없는 새로운 인물·사건·대사·날짜를 추가하지 마세요.
- 완성된 재조립본 전체만 출력하세요. 안내 문구나 수정 내역 설명을 출력에
  포함하지 마세요 — 이 출력이 그대로 session_prose로 저장됩니다.
"""


def build_distortion_repair_prompt(
    *, reassembled_prose: str, original_text: str, flags: list[dict[str, str]]
) -> list[dict[str, str]]:
    """flags: [{"sentence": 문제 문장, "reason": 사유}, ...] —
    DISTORTION_CHECK_SCHEMA로 판정한 결과를 그대로 넘긴다(build_chapter_repair_prompt의
    flagged_items와 같은 모양)."""
    flags_block = "\n".join(
        f"- 문장: {item['sentence']}\n  사유: {item['reason']}" for item in flags
    )
    return [
        {"role": "system", "content": DISTORTION_REPAIR_SYSTEM_PROMPT},
        {
            "role": "user",
            "content": (
                f"[재조립본]\n{reassembled_prose}\n\n"
...
# 이 구역(8~9절)의 프롬프트는 app/services/autobiography_service.py와
# app/services/character_service.py에서만 쓰이며, app/clients/llm_router.py를 통해
# Solar, Gemini, Claude 중 하나로 라우팅된다(settings.AUTOBIOGRAPHY_LLM_PROVIDER). 새
# 사실을 "생성"하는 프롬프트(목차·시놉시스·챕터 집필·수리·윤문 등)에는 "화자가 실존
# 인물이어도 학습·검색으로 아는 실제 정보를 가져오지 말라"는 가드를 반드시 넣는다 —
# Solar 대비 실세계 지식(특히 유명인)이 훨씬 강한 Gemini·Claude로 전환했을 때, 제공된
# 근거 자료가 아니라 모델이 "이미 알고 있는" 실제 인물 정보로(또는 Claude 특유의
# "도움이 되려는" 성향으로 사실을 보완하려다가) 빈틈을 메우는 위험이 커지기 때문이다
# (이 프로젝트가 시험 계정으로 쓰는 billgates 같은 실존 유명인 이름이 대표적인 노출
# 사례). 판정 프롬프트(근거검증 등)에는 반대 방향의 가드 — "네가 아는 실제 정보와
# 대조하지 말고 오직 제공된 근거 자료로만 판단하라" — 를 넣는다.
...
사실 관계: 화자가 실존 인물이거나 유명인과 이름이 같더라도, 당신이 사전 학습이나
검색으로 알고 있는 그 인물의 실제 경력·사건·인간관계를 절대 목차 설계에 끌어오지
마세요 — narrative_arc·part_arc·connecting_thread가 근거로 삼을 수 있는 것은 오직
아래 제공된 사건 요약 목록뿐입니다. 목록에 없는 사건을 아는 척 목차에 반영하면
이후 모든 집필 단계가 그 위에 지어져 사실과 다른 책이 만들어집니다.
"""
...
있는 범위를 넘지 않아야 합니다. 화자가 실존 인물이거나 유명인과 이름이
같더라도, 당신이 사전 학습이나 검색으로 알고 있는 그 인물의 실제 정보(경력,
사건, 인간관계, 발언 등)를 채워 넣지 마세요 — 이 자서전 안에서 사실로 인정되는
것은 오직 위 목차에 적힌 내용뿐입니다.

이 결과물은 그대로 책의 소개 페이지에 인쇄됩니다 — "# 시놉시스" 같은 마크다운
제목이나 『책 제목』 같은 표제를 앞에 붙이지 말고, 완성된 소개글 산문만
출력하세요. 책 제목은 이미 표지에 별도로 인쇄되므로 여기서 다시 언급할
필요가 없습니다.
"""
...
말고 감정·주제 흐름만 추상적으로 서술하세요. 화자가 실존 인물이거나 유명인과
이름이 같더라도, 당신이 사전 학습이나 검색으로 알고 있는 그 인물의 실제 정보로
근거 사건 요약의 빈틈을 메우지 마세요 — 아무리 사실일 가능성이 높아 보여도
근거 사건 요약에 없으면 이 책 안에서는 없는 일입니다.
...
사실 관계: 구체적 장소·사물·인물·사건은 반드시 [배정된 사건 목록]에 실제로
있는 내용만 쓰세요. 화자가 실존 인물이거나 유명인과 이름이 같더라도, 당신이
사전 학습이나 검색으로 알고 있는 그 인물의 실제 정보를 끌어와 장면을 채우지
마세요 — 근거 사건 목록에 없으면 이 챕터에서는 없는 일입니다.
...
가장 먼저 지킬 것: 화자가 실존 인물이거나 유명인과 이름이 같더라도, 당신이 사전
학습이나 검색으로 알고 있는 그 인물의 실제 정보(경력·사건·인간관계·발언 등)를
이 챕터에 끌어오는 것은 어떤 경우에도 금지입니다. 이 책 안에서 사실로 인정되는
것은 오직 아래 [RAG 검색된 사건 문단]뿐이며, 이 규칙은 이어지는 다른 모든
지시보다 우선합니다(아래 "사실 관계" 절에서 다시 자세히 다룹니다).

아래 자료를 바탕으로 챕터 본문을 집필하세요. [스타일 바이블]의 문체를 따르고,
[전체 시놉시스]와 [챕터 시놉시스]의 설계를 벗어나지 마세요.

분량: 장면 수·문단 수·문장 수를 기계적으로 채우려 하지 마세요(2026-07-19 —
이전의 "장면 6~7개×문단 8개×문장 4개" 같은 산술적 하한이, 근거가 얇은
사건까지 억지로 늘리려다 화자가 언급하지 않은 감각 묘사·비유를 지어내는
원인으로 실측 확인돼 폐기했다). [RAG 검색된 사건 문단]에 담긴 정보의 밀도와
무게에 비례해 자연스러운 분량으로 쓰세요 — 화자가 그 사건에 대해 충분히
이야기했다면 장면·감각·내적 성찰로 깊이 풀어 쓰고, 단편적으로만 언급됐다면
담백하게 서술한 뒤 다음 사건으로 넘어가세요. 짧더라도 근거에 충실한 챕터가,
없는 디테일로 억지로 늘린 챕터보다 낫습니다.
...
- "그리고 -했다. 그리고 -했다" 식의 사건 나열이나 요약형 진술로 열지 마세요.
  [RAG 검색된 사건 문단]에 실제로 담긴 장면·감각(공간, 소리, 냄새, 몸짓)
  디테일이 있다면 그것으로 문을 열되, 없는데 분량이나 분위기를 위해
  냄새·소리·촉감 같은 감각 묘사를 지어내 끼워 넣지는 마세요 — 화자가
  언급하지 않은 디테일을 채워 넣는 것은 정교화가 아니라 날조입니다.
- 감정을 이름 붙여 설명("슬펐다", "기뻤다")하기보다, 행동과 디테일로 그 감정이
  드러나게 하세요(보여주기, showing not telling). 단, 이때의 "디테일"도 근거
  사건에 실제로 있는 것이어야 합니다 — 없는 행동·표정·물건을 소설처럼
  창작하지 마세요.
- 문단 연결: 각 문단은 바로 앞 문단에서 인과, 시간의 흐름, 정서적 여운 중
  ...
  필요 없습니다. [RAG 검색된 사건 문단]의 문장을 그대로 옮기거나 순서대로
  이어 붙이지 마세요("위키백과 요약본"처럼 무미건조해지는 원인입니다 — 예:
  "1963년 진단을 받았다"와 "가장 뼈아픈 좌절은 1963년 진단이었다"처럼 같은
  정보를 한 문단 안에서 다른 말로 반복하는 것도 이 문제의 증상입니다). 근거에
  없는 사실을 지어내라는 뜻이 아니라, 이미 주어진 사건들 사이의 인과관계와
  화자의 내면적 감정 흐름을 매끄럽게 엮어 이력서식 사건 나열이 아닌 한 편의
  이야기로 구성하라는 뜻입니다. 단, "엮으라"는 것은 문장과 문장 사이의
  흐름을 자연스럽게 만들라는 뜻이지, 마침표를 지우고 여러 문장을 하나의
  문장으로 물리적으로 합치라는 뜻이 절대 아닙니다(2026-07-19 실사용 중
  확인된 사고 — "예감을 느꼈다 그는 칠판에..."처럼 문장 종결 부호가 통째로
  사라지는 증상). 각 문장은 반드시 마침표(또는 물음표·느낌표)로 명확히
  끝내고 그다음 문장을 새로 시작하세요.
- 문단 길이: 챕터 전체를 문단 구분 없이 하나로 쓰지 마세요 — 장면이나 생각의
  단위가 바뀔 때 문단을 나누세요. 반대로 한두 문장마다 줄바꿈해 문단을 잘게
  쪼개지도 마세요(발표 자료의 총알 기호처럼 뚝뚝 끊겨 읽힙니다). 문단 하나에는
  보통 여러 문장이 자연스럽게 이어져야 합니다.
- 시점·어조 일관: 처음부터 끝까지 1인칭("나")을 유지하세요 — 자기 자신을
  3인칭("그", 이름)으로 부르는 문단이 섞이면 안 됩니다. 종결어미는 [스타일
  바이블]의 "종결어미 규정"이 지정한 하나로 챕터 끝까지 유지하세요(사용자가
  선택한 말투 지시가 있으면 그쪽이 우선). [RAG 검색된 사건 문단]은 화자가
  실제로 말한 원문 그대로라 존댓말("-습니다", "-어요")이나 다른 어미로 되어
  있을 수 있습니다 — 그 문장을 어미까지 그대로 옮기지 말고, 내용만 가져와
  반드시 [스타일 바이블]이 지정한 종결어미로 새로 쓰세요. 원문의 어미가
  그대로 섞여 들어가 한 챕터 안에서 존댓말과 반말이 오가면 안 됩니다.
...
# 챕터 분량 확장 패스는 2026-07-19 폐기됐다 — 실측 결과 확장 호출도 "원본보다
# 길어지기만 하면 채택"이라는 약한 기준뿐이라 3,000자 문턱조차 보장하지 못했고,
# 챕터당 이미 여러 번인 순차 Solar 호출에 매번 1회를 더 얹어 처리 속도를 크게
# 늦추기만 했다(집필 5~10분/챕터, 40분에 4장 실측). 부족한 분량을 재호출로
# 사후 보정하는 대신 CHAPTER_WRITING_SYSTEM_PROMPT의 "분량" 절 자체를 장면 수·
# 문단 수·문장 수 기준(모델이 실제로 세면서 지킬 수 있는 단위)으로 다시 써서
# 첫 호출에서 바로 채우도록 했다. 여전히 짧게 나오는 극소수 챕터는 사용자가
# '나의 자서전'의 직접 수정 기능으로 보완하면 된다 — 매번 LLM 재호출을 태우는
# 것보다 싸다.
...
3. 종결어미 격식 혼입: 본문 전체의 지배적인 종결어미(예: "-다"체)와 다른
   격식(예: "-습니다"체)이 섞인 문장을 지배적인 쪽으로 통일하세요. 단,
   어미 글자만 기계적으로 덮어씌우지 마세요 — 그렇게 하면 "이어지며다",
   "품게 하다", "밀어내며다"처럼 조사·어미가 뒤엉킨 비문이 생깁니다
   (2026-07-19 실사용 중 확인). 문장 전체의 호응을 확인해, 필요하면 어미
   앞부분까지 자연스럽게 고쳐 쓰세요.
...
출력: 교열이 끝난 본문 전문만 출력하세요. 고친 부분 목록, 설명, 안내 문구,
마크다운 문법(**굵게**, ### 제목, - 목록 등)을 붙이지 마세요 — 순수 텍스트
줄바꿈과 문단 구분만 유지된 본문이어야 합니다.
"""
...
화자가 실존 인물이거나 유명인과 이름이 같더라도, 당신이 사전 학습이나 검색으로
알고 있는 그 인물의 실제 정보와 원고 내용이 다르게 보인다는 이유로 "바로잡지"
마세요 — 이 원고 안에서 사실로 인정되는 것은 오직 원고에 이미 적힌 내용뿐이며,
당신의 배경지식과 대조해 날짜·장소·인물 관계 등을 고치는 것은 윤문의 범위를
벗어난 임의 개작입니다.
...
판정은 오직 [근거 사건 목록]과의 대조로만 하세요. 화자가 실존 인물이거나
유명인과 이름이 같더라도, 당신이 사전 학습이나 검색으로 알고 있는 그 인물에
대한 실제 정보를 근거로 판정에 쓰지 마세요 — 어떤 문장이 실세계에서 사실일
가능성이 높아 보인다는 이유로 통과시키면 안 됩니다. 이 판정의 목적은 "실제로
있었던 일인가"가 아니라 "제공된 근거 사건 목록으로 뒷받침되는가"이므로, 근거
목록에 없으면 당신이 알기로 사실이더라도 반드시 flag하세요.
...
- 수정하면서 근거 사건 목록에 없는 새로운 사실·인물·사건을 추가하지 마세요.
  화자가 실존 인물이거나 유명인과 이름이 같더라도, 당신이 사전 학습이나
  검색으로 알고 있는 그 인물의 실제 정보로 빈 자리를 채우지 마세요 — 근거
  사건 목록에 없으면 이 책 안에서는 없는 일입니다.
