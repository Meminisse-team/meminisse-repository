...
# 안 틀리고 있는 문장을 "원본에 없다"고 판정한 사례). 지적 사유를 곧이곧대로
# 믿고 실행하면 실제로는 화자가 한 말을 지워버리는, 이 함수의 존재 목적과
# 정반대인 결과가 나온다 — 그래서 수리 모델이 삭제/수정 전에 [원본 발화]를
# 스스로 다시 대조해 판정 자체를 기각할 수 있는 여지를 명시적으로 준다.
DISTORTION_REPAIR_SYSTEM_PROMPT = """\
아래 [재조립본]은 [원본 발화]를 1인칭 산문으로 정리한 것인데, 검증 과정에서
[지적 사항]에 적힌 문제가 발견됐다고 합니다. 하지만 그 지적이 틀렸을 수도
있습니다 — 판정자도 실수할 수 있으므로, 그대로 믿지 말고 직접 확인하세요.

0단계 — 먼저 반드시 확인할 것: [지적 사항]이 가리키는 내용을 [원본 발화]
전체에서 실제로 찾아보세요(표현이 살짝 다듬어졌을 수 있으니 뜻이 같은지로
판단하세요). **[원본 발화]에 그 내용(또는 같은 뜻의 내용)이 실제로 있다면,
그 판정은 오탐입니다 — 그 부분은 절대 삭제하거나 고치지 말고 [재조립본]을
한 글자도 바꾸지 않은 채 그대로 출력하세요.**

[원본 발화]를 확인한 결과 지적이 정말 맞다면(그 내용이 원본 어디에도 없다면)
그때만 아래 방법으로 고치세요:
1. [원본 발화]에 실제로 있는 내용으로 바꿔 쓸 수 있으면, 원본에 맞게 고쳐
   쓰세요(문체와 흐름은 주변 문장과 자연스럽게 이어지도록).
2. 원본에 대응하는 내용이 전혀 없으면 그 부분을 삭제하고, 필요하면 앞뒤
   문장을 최소한으로 다듬어 흐름이 끊기지 않게 하세요.

반드시 지킬 것:
- [지적 사항]과 무관한 문장은 한 글자도 바꾸지 마세요 — 이 작업은 전면
  재작성이 아니라 국소 수술입니다. 지적되지 않은 내용을 요약하거나
  생략하거나 표현을 바꾸지 마세요.
...
def build_distortion_repair_prompt(
    *, reassembled_prose: str, original_text: str, fail_reason: str
) -> list[dict[str, str]]:
    return [
