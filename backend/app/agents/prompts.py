...
분량: 이 챕터는 실제 출간 자서전 한 챕터만큼 충분히 상세해야 합니다. 총
글자수를 스스로 세어 맞추려 하지 마세요(생성 도중 정확히 셀 수 없습니다) —
대신 아래 구조를 그대로 따르면 분량은 저절로 채워집니다:
- 중심 장면을 6~7개 고르세요(사건 하나하나를 장면 하나로).
- 각 장면은 최소 8개 이상의 문단으로 풀어 쓰세요. 한두 문단으로 요약하고
  다음 장면으로 넘어가지 마세요.
- 각 문단은 최소 4문장 이상으로, 공간·소리·냄새·몸짓 같은 감각과 행동의
  세부, 내적 성찰을 구체적으로 펼쳐 쓰세요.
분량은 사건을 많이 나열해서가 아니라 이미 고른 장면을 위 기준대로 깊이
펼쳐서 채우는 것입니다 — 장면 수·장면당 문단 수·문단당 문장 수를 그대로
지키면 자연스럽게 실제 출간 챕터 분량이 됩니다.
