...
from app.clients import embeddings, groundedness, nli, solar
...
async def _passes_distortion_check(
    *, original_turns: list[dict[str, str]], reassembled_prose: str
    ...
    않았는지 Solar LLM 판정으로 확인한다. 반환값은 (통과 여부, flags) — flags는
    [{"sentence": 문제 문장, "reason": 사유}, ...] 형태로, _repair_distorted_prose의
    외과적 수리 입력으로 그대로 쓰인다(GROUNDEDNESS_JUDGE_SCHEMA와 동일한 구조).
    ...
    처음엔 단문 프로토콜(PASS / FAIL: 사유 한 문장)을 썼는데(solar-mini의 Structured
    Outputs 지원이 검증된 적 없다는 이유), 실사용 중 (1) 사유가 산문 전체를
    가리키는 문장 하나뿐이라 어느 문장이 문제인지 특정이 안 돼 수리가 부정확했고,
    (2) 산문 전체를 한 번에 보고 하나의 판정만 내리다 보니 오탐·미탐이 둘 다
    나왔다(2026-07-19). solar-mini가 이 스키마의 Structured Outputs를 실제로
    지원함을 실측으로 확인해(2026-07-19) 문장 단위 flags 배열로 바꿨다.

    문장 단위로 바꾼 뒤에도 solar-mini가 원본에 토씨 하나 안 틀리고 있는 문장을
    "지어냈다"고 확신에 차서 오판하는 사례가 반복 재현됐다(2026-07-19, 세션
    911fbf5d — 같은 오탐이 재조립을 5라운드 다시 만들어도 매번 나옴). 그래서
    _confirm_distortion_flags로 2차 게이트를 추가했다 — autobiography_service.
    _run_groundedness_check와 정확히 같은 패턴이지만 모델 배치는 반대다: 그쪽은
    1차 판정이 pro3(챕터 집필과 같은 모델)라 2차 확인을 mini로 둬 자기선호
    편향을 피했고, 여기는 1차 판정이 mini이므로 2차 확인을 pro3로 둬 같은
    이유로 반대 방향의 편향(mini 자신의 오판을 mini가 또 확인하는 것)을 피한다.
    """
    ...
    result = await solar.structured_completion(
        ...
        schema_name="distortion_check",
        json_schema=prompts.DISTORTION_CHECK_SCHEMA,
        model=_DISTORTION_JUDGE_MODEL,
        # solar-mini는 추론 모델이 아니라 reasoning_effort 파라미터 자체를 거부한다
        # (Upstage 400: "reasoning_effort parameter is only supported for reasoning
        # models", 2026-07-19 실측 — structured_completion의 기본값이 "medium"이라
        # 아무것도 안 넘기면 이 오류가 난다). clients/groundedness.py의 solar-mini
        # 호출도 애초에 reasoning_effort를 넘기지 않는데, 단순히 비용 절감이 아니라
        # 이 제약 때문이었을 가능성이 높다.
    ...
    flags = await _confirm_distortion_flags(flags=flags, original_text=original_text)
    if not flags:
        return True, []

    logging.getLogger(__name__).warning(
        "산문 재조립 왜곡 탐지 실패(solar-mini, 2차 게이트 통과 %d건): %s", len(flags), flags
    )
    return False, flags


async def _confirm_distortion_flags(
    *, flags: list[dict[str, str]], original_text: str
) -> list[dict[str, str]]:
    """왜곡 탐지 2차 게이트: 1차 판정(solar-mini)이 지목한 각 문장을 solar-pro3로
    독립적으로 다시 확인해, "원본에 실제로 있다(grounded)"고 명시적으로 확정된
    것만 오탐으로 철회한다. clients/groundedness.py를 그대로 재사용한다(그
    모듈의 solar-pro3 오버라이드 경로는 evals/groundedness_gate_accuracy.py가
    이미 정확도 비교용으로 실측 검증해둔 코드 경로다).

    검증 실패(notGrounded/notSure/호출 예외)는 전부 플래그 유지로 보수적으로
    처리한다 — "검증 실패가 검증 통과로 둔갑하면 안 된다"는 원칙은
    autobiography_service._run_groundedness_check의 2차 게이트와 동일하다."""

    async def _confirm(flag: dict[str, str]) -> dict[str, str] | None:
        try:
            verdict = await groundedness.check(
                context=original_text, answer=flag["sentence"], model="solar-pro3"
            )
        except Exception as exc:  # noqa: BLE001
            logging.getLogger(__name__).warning(
                "왜곡 탐지 2차 게이트(solar-pro3) 호출 실패(플래그 유지): %s", exc
            )
            return flag
        return None if verdict == groundedness.GROUNDED else flag

    confirmed = await asyncio.gather(*(_confirm(flag) for flag in flags))
    dismissed = sum(1 for item in confirmed if item is None)
    if dismissed:
        logging.getLogger(__name__).info(
            "왜곡 탐지 2차 게이트(solar-pro3)가 %d건의 1차 판정을 오탐으로 철회함", dismissed
        )
