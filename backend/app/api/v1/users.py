...
async def create_consent(user_id: uuid.UUID, payload: ConsentCreate, db: DbSession) -> ConsentRead:
    """
...
async def get_consents(user_id: uuid.UUID, db: DbSession) -> list[ConsentRead]:
    records = await consent_service.list_consents(db, user_id)
